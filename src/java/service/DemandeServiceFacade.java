...
public class DemandeServiceFacade extends AbstractFacade<DemandeService> {
    ...
    public List<DemandeService> findDemandesByWorker(Worker worker) {
        String requete = "SELECT ds FROM DemandeService ds WHERE ds.worker.login='" + worker.getLogin() + "'";
        String requetteVoiture = "SELECT dv.demandeService FROM DemandeVoiture dv,DemandeVoitureItem dvItem  WHERE dvItem.demandeVoiture.id=dv.id AND dvItem.worker.login='" + worker.getLogin() + "'";

        List<DemandeService> demandeServices = getMultipleResult(requete);
        List<DemandeService> demandeServicesVoiture = getMultipleResult(requetteVoiture);
        demandeServices.addAll(demandeServicesVoiture);
        return demandeServices;
    }

    public List<Client> findClientByWorker(Worker worker) {
        String requete = "SELECT ds.client FROM DemandeService ds WHERE ds.worker.login='" + worker.getLogin() + "'";
        return em.createQuery(requete).getResultList();
    }

    public int findNumberOfDemandesByWorker(Worker worker) {
        List<DemandeService> demandes = getMultipleResult("SELECT ds FROM DemandeService ds WHERE ds.worker.login='" + worker.getLogin() + "' AND ds.dateConfirmation = NULL");
        if (demandes == null) {
            return 0;
        }
    ...
    public List<DemandeService> rechercher(Worker worker, Client clientRecherche, Service serviceRecherche, int etatRecherche, Date dateMin, Date dateMax) {
        ...
        String requete = "SELECT ds FROM DemandeService ds WHERE 1=1";// ds.service.id != 21";

        if (worker != null && worker.getLogin() != null) {
            requete += " and ds.worker.login = '" + worker.getLogin() + "'";
        }
