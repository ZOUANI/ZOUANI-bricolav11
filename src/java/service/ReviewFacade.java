...
public class ReviewFacade extends AbstractFacade<Review> {
    ...
    public double calculRating(Worker worker) {
        double stars = 0;
//        List stars = em.createQuery("SELECT r.stars FROM Review r WHERE r.worker.login='"+worker.getEmail()+"'").getResultList();
        List<Review> reviews = getMultipleResult("SELECT r FROM Review r WHERE r.worker.login='" + worker.getLogin() + "'");
        if (reviews == null) {
            return stars;
        } else {
    ...
    public List<Review> findByCriteria(String societe,Long service){
       String query = "select r from Review r where 1=1 ";
        query+=SearchUtil.addConstraint("r", "worker.login", "=", societe);
    ...
    public List<Review> findReviewByWorker(Worker selected) {
        ...
        String requette = "select r from Review r where r.worker.login = '"+selected.getLogin()+"'";
