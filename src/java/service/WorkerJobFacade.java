...
public class WorkerJobFacade extends AbstractFacade<WorkerJob> {
    ...
    public int findNumberofServicesByWorker(Worker worker) {
        List<WorkerJob> jobs = getMultipleResult("SELECT wj FROM WorkerJob wj WHERE wj.worker.login='" + worker.getLogin() + "'");
        if (jobs == null) {
            return 0;
        }
        return jobs.size();
...
//        return getMultipleResult("SELECT wj.service FROM WorkerJob wj WHERE wj.worker.login='"+worker.getEmail()+"'");
//        return em.createQuery("SELECT wj.service FROM WorkerJob wj WHERE wj.worker.login='"+worker.getEmail()+"'").getResultList();
...
//                + "w.service.nom = '" + service + "' and w.worker.workerType.id = '" + type + "'";
    ...
    public List<WorkerJob> findByWorker(Worker worker) {
        ...
        String requette = "select w from WorkerJob w where w.worker.login='" + worker.getLogin() + "'";
        System.out.println("hahiya requette : " + requette);
    ...
    public void create(Worker worker, Service service,Secteur secteur) {
        WorkerJob item = new WorkerJob();
        item.setId(generateId("WorkerJob", "id"));
        item.setWorker(worker);
        item.setService(service);
        item.setSecteur(secteur);
        create(item);
    }
}
