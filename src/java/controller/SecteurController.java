...
public class SecteurController implements Serializable {
    ...
    private Ville villeRecherche;

    public void findByVille(){
        items= ejbFacade.findByVille(villeRecherche);
        getSelected().setVille(villeRecherche);
    }
    public Ville getVilleRecherche() {
        return villeRecherche;
    }

    public void setVilleRecherche(Ville villeRecherche) {
    ...
    public Secteur prepareCreate() {
        selected = new Secteur();
        selected.setVille(villeRecherche);
    ...
    public void create() {
        ...
        if (!JsfUtil.isValidationFailed()) {
            findByVille();    // Invalidate list of items to trigger re-query.
        }
    }
